...
def evaluate(ref_file, trans_file, metric, bpe_delimiter=None):
    ...
    elif metric.lower() == "moses_bleu":
        evaluation_score = _moses_bleu(ref_file, trans_file,
                                  bpe_delimiter=bpe_delimiter)
...
def _moses_bleu(tgt_test, trans_file, bpe_delimiter=None):
    ...
    multi_bleu_script ='multi-bleu-detok.perl'
    cmd = "%s %s %s < %s" % ('perl',multi_bleu_script, tgt_test, trans_file)
    # subprocess
